...
value is owned. A stable notation prefix fused into the same alphanumeric risk token is part of
its formatting frame: keep complete `UN1197` owned by its `unNumber` binding, while the whitespace-
separated `UN` in `UN 1197` may remain literal. Never request a narrowing that leaves part of the
same risk token uncovered. Stable caption and modality text is not a missing part of the value. For example,
...
1. No shipment-specific, private, auxiliary, repeated, calculated, party, route, transport, cargo,
   ...
   captions may remain literal when the form prints multiple alternatives. In particular,
   `ORIGINAL` inside `NUMBER OF ORIGINAL BILLS OF LADING` is caption grammar, not a repeat of a
   standalone selected original-status mark.
...
7. Only `deterministic_derived` declares a derivation or dependencies. Use it when the rendered
   ...
   A package quantity and category printed together are two disjoint direct leaf owners;
   `package_count` means collection cardinality and never combines scalar `quantity` and
   `typeCategory` leaves.
