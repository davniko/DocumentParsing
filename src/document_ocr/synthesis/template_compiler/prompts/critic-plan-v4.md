...
- Every decoded `occurrenceTable` record is named and carries its original zero-based
  `occurrenceRowIndex`. Current occurrence removal IDs are exact line-addressed
  `occ_Lxxxxx_xxxxx` handles copied from that table.
...
- Every decoded `candidateTable` record carries its original zero-based `candidateIndex` and its
  exact host-issued `candidateId`. The index is only an audit selector. For every required
  `cross_field_semantic_relation`, return exactly one `coherence_decisions` row and copy its full
  `candidateId` byte-for-byte. Never construct `candidate_N`, substitute the numeric index, or
  emit coherence decisions for any other candidate kind.
- Every decoded `occurrenceCandidateTable` record is named. Prefer an exact supplied
  `{"occurrence_id":"cand_Lxxxxx_xxxxx"}` handle instead of copying a literal occurrence whenever
  the intended exact surface is listed. Never construct or renumber a handle.
  `currentExactOwnerLogicalKeys` is the complete existing
...
- never narrow a fused notation token such as `UN1197` to `1197`; the stable `UN` formatting frame
  must remain owned with the target value so the same risk token stays complete;
...
For a current occurrence removal, use only an exact `occ_Lxxxxx_xxxxx` from `occurrenceTable`,
under its actual owner. For an added occurrence, use only an exact listed `cand_Lxxxxx_xxxxx`
handle unless the
...
- A package quantity and category printed together remain two disjoint direct leaf owners.
  `package_count` means collection cardinality; it never derives a scalar package `quantity` plus
  `typeCategory` surface.
...
- `ORIGINAL` inside an original-bill-count caption is fixed caption grammar. It is not another
  occurrence of a standalone original-status mark.
