...
- Derived totals and counts are declared as deterministic derivations rather than copied or
  ...
  A printed quantity-plus-kind surface such as `1 PARCEL` or `360 BOXES` is covered by disjoint
...
- A word such as `ORIGINAL` inside `NUMBER OF ORIGINAL BILLS OF LADING` is caption grammar, not a
  repeated original-status mark. Only a standalone selected mark may join an original-status
  binding.
- A stable alphanumeric notation prefix fused into one risk token is part of its formatting frame,
  not a separable caption. In particular, keep the complete `UN1197` token owned by the structured
  `unNumber` binding; narrowing it to `1197` leaves `UN` uncovered and is invalid. Expanded
  `UN 1197` may keep `UN` literal because whitespace separates the notation from the value. Apply
  this distinction consistently in the first review; never request a narrowing that the risk
  contract would have to reverse on the next pass.
