...
Equipment, reefer status, temperature, volume, or free-time facts absent from both labels but
printed in source OCR are unextracted template flavor. Target-label absence does not mean dry cargo
or authorize deleting that topology. Require coherent new shipment-specific values while
preserving the source equipment family and operational relationship grammar.
...
5. `legalTopology`: Legal and agency relationships remain, use the exact target primary party, and
   ...
   consistent fictional agent with local wrapping and `By` placement intact. Reject a rewritten
   agent that retains a source-only jurisdictional legal suffix, country designator, or registration
   scheme that conflicts with the supplied export/issue context.
6. `formattingAndMinimality`: Page/section order, headings, line count, blank-line mask, populated
   ...
   Numbered structural prefixes such as `(23)` and `7.3.` remain byte-identical on their original
   line.
...
- Distinct target primary parties must not share an invented auxiliary company, affiliation,
  relationship phrase, person, ID, or subordinate address detail. A source-shared relationship may
  repeat only when the corresponding target party identity is also shared.
...
- For raw-only reefer/equipment topology, accept a capacity-sufficient synthetic count, coherent
  non-source volume/free-time values, and the original compatible reefer boilerplate with only stale
  named entities or dependent numbers changed. Temperature-sensitive cargo needs a plausible
  non-source setpoint; non-temperature cargo needs an explicit non-operating/ventilation instruction
  without a setpoint. Reject placeholders and conversion to dry/general-purpose solely because the
  target label lacks a container field. Do not request stylistic paraphrases of compatible prose.
- If the source setpoint is removed, reject any surviving positive powered-operation phrase such
  as `Plugging for the account of cargo`. It contradicts a non-operating/no-setpoint rewrite and
  must become coherent same-slot ventilation or non-operating flavor without deleting the slot.
- An explicitly headed HS/commodity-code slot is extractable target truth. Require the exact
  prefilled target surface supplied by target integrity; reject any independently invented code,
  stale source code, handling prose, or needless change to its heading and delimiter grammar.
...
- In generic liability/operational boilerplate, replacing a stale short carrier brand with
  source-style `the Carrier` is correct. Do not demand another full target carrier occurrence in
  that prose; exact role-bound party occurrence requirements remain authoritative.
