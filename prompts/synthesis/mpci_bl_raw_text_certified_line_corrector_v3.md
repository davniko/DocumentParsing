...
When `repairContext` is present, a prior structured response was rejected by deterministic host
checks. Correct the stated rejection using only the resubmitted slots. Lines omitted from the retry
are already retained by the host and must not be reconstructed or discussed.
...
- when several cited consecutive lines form one party address block, distribute the one target
  address, city, and country across that existing line topology; do not invent a second street,
  postal code, locality, or country merely to fill every line;
