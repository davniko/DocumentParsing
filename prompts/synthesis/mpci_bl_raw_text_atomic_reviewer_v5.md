...
- Every `operationalFlavorRequirement` is exact deterministic authority. Its named source line must
  contain the supplied target measurement surface with the original local grammar; do not ask for
  a different plausible value or accept the old source surface. The value is already sampled from
  paired real B/L utilization evidence and bounded by the synthetic container capacity.
