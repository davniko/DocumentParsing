...
- `operationalFlavorRequirements` remove discretion from raw-only container measurements. Put each
  exact `targetValueSurface` on its named `sourceLineId`, retaining its measurement label, unit,
  punctuation, decimal/grouping shape, and surrounding text byte-for-byte. These values were
  resampled as paired real-document capacity utilizations and scaled to the synthetic equipment;
  never retain the `sourceValueSurface`, choose a different value, or move it to another line.
