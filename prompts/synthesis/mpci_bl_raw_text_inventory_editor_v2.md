...
- The payload's `partyBlocks` groups all slots owned by one role. Across each group, reproduce
  ...
  remaining line with realistic same-role auxiliary flavor rather than a placeholder.
...
- An `auxiliaryIdentityConsistencyGroups` entry represents repeated copies of one source-only
  ...
  Generate one distinct fictional identity and reuse it exactly in every listed slot; do not
  create a different company for each repeated copy.
