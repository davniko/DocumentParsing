...
- Render semantic category tokens as plausible surfaces in the template's style; do not print the
  schema token itself.
- Preserve legal relationship structure while replacing every identity within it. If the target
  lacks the identities needed to preserve that structure without corrupting label truth, block.
