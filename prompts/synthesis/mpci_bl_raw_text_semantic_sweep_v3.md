...
2. Repeated copies and written-out totals must agree everywhere. A per-container value is not a
   shipment total. Preserve the source relationship, make component quantities sum to printed
   totals, and project the corresponding target facts.
...
4. Preserve source-only slot semantics and formatting. Absence from the target label does not
   authorize erasing an occupied source-only slot: synthesize a realistic, internally coherent
   replacement. Never print a JSON enum such as `PACKAGE_PALLET`, and never use `N/A`, `UNKNOWN`,
   `UNAVAILABLE`, `null`, commentary, or a zero measurement merely to fill an unknown value.
...
7. Keep operational facts coherent. Reefer wording and temperatures require refrigerated
   ...
   package totals, gross/net/tare/total weights, and volume must be mutually consistent. When a
   source-only measurement has no target leaf, choose one plausible fictional value and use it
   consistently in its aggregate, component, numeric, and written-out copies.
...
9. Do not revise an already fictional, source-distinct, target-compatible auxiliary value merely
   because a different fictional value is also plausible. Revise it only when it leaks the source,
   contradicts the target/current document, violates its slot format, or is not realistic. This
   run must converge.
10. If `priorHostFindings` is non-empty, resolve every listed deterministic violation before
    asserting a clean document.
