...
- a standalone `NON-NEGOTIABLE COPY` stamp describes the presented copy, not a different carriage
  ...
  `bill_of_lading`; derive negotiability from the completed consignee construction and specific
  contract terms rather than the copy stamp or generic title boilerplate. A named non-order
  consignee is a straight, non-negotiable bill, while an explicitly completed `TO ORDER`
...
- A contiguous pallet-number partition is a special, deterministic nested-package form. If
  source-ordered rows such as `Pallet No. 1 - 4`, `Pallet No. 5 - 6`, ... begin at 1, do not
  overlap or leave gaps, and reconcile exactly to the pallet total printed for one identified
  container, emit one cargo group per row. Emit the inclusive range cardinality as the outer
  `PALLET` package quantity and the row's printed drums/tinplate containers/cartons as its inner
  package level; attach the row's own net/gross measures. Link the outer pallet level to that one
  container with `single_package_level`. Do not also emit the shipment pallet total as a duplicate
  package fact, and do not synthesize or duplicate the range/containment wording in
  `additionalInformation`. If any precondition fails, do not apply this rule.
...
- Normalize an explicitly headed date to ISO 8601. `issueDate` requires issue/date-of-issue context,
  ...
  language, party nationality, or other geographic locality. Whenever both leading numeric
  components are 12 or below, apply the dataset's deterministic day-first convention directly: the
  first component is day and the second is month (`01/04/2024` becomes `2024-04-01`;
  `12.07.2024` becomes `2024-07-12`). Apply this to the ambiguous token even if another date in the
  same document uses a different numeric order. This is not an ambiguity warning and is never
  grounds to omit the date or hold the document from training. Preserve the exact printed form in
  evidence.
...
- `forwardingAndExportReferences` contains value-only data under an explicit forwarding/export
  ...
  Explicit empty placeholders such as `NO REF`, `N/A`, `NONE`, or `NIL` are absence and must not
  be emitted as reference values.
...
- A standalone `NON-NEGOTIABLE` statement that is not the copy-status phrase
  ...
  document term.
- `flashPoint` requires either an explicit `FLASH POINT`/`FLASH PT`/`FLP` label or a printed
  temperature carrying `C.C.`, `C-CC`, or `CLOSED CUP` notation in the same local dangerous-goods
  block (for example beside a UN number, hazard class, or packing group). This closed-cup notation
  is the printed flash-point method. Never treat an ordinary cargo, storage, or reefer temperature
  as flash point. Preserve an explicitly printed packing group in a representable field only;
  never manufacture a temperature merely to unlock the schema.
...
- A completed named consignee that is not a `TO ORDER` construction establishes a straight
  `negotiability=non_negotiable` B/L. Generic preprinted positive-original, surrender,
  one-accomplished/others-void, or `delivered unto order or assigns` boilerplate does not override
  the completed straight consignment. Use the named consignee (plus any applicable conditional
  term) as evidence; do not use `NON-NEGOTIABLE COPY` alone. An explicitly completed `TO ORDER`
  consignee establishes `negotiability=negotiable`, while explicit sea-waybill/express terms still
  govern their document.
...
Apply this rule before returning:

- In a one-cargo-group document, explicit goods/marks wording `MADE IN <country>` is cargo-origin
  evidence. Populate `cargoGroups[0].origin.name` with the country exactly as printed; do not
  substitute a code, infer the origin from a party address, or leave this explicit value null.
