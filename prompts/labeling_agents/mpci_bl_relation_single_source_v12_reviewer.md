...
- exactly one eligible maritime carriage document; all pages are in source order. Eligible documents
  ...
  The completed term `EXPRESS RELEASE - NO ORIGINALS ISSUED` requires
  `negotiability=non_negotiable`; generic surrender boilerplate does not override it.
  ...
  original-surrender/title terms establish the carriage contract. Retain the `bill_of_lading`
  document type, but derive negotiability from the completed consignee construction and specific
  contract terms rather than the copy stamp or generic title boilerplate: a named non-order
  consignee is straight/non-negotiable, while an explicitly completed `TO ORDER` construction is
  negotiable.
...
- `issueDate` is explicitly an issue/date-of-issue value and `shippedOnBoardDate` is explicitly a
  ...
  order evidence. Whenever both leading numeric components are 12 or below, require the dataset's
  deterministic day-first convention directly: first component is day and second is month
  (`01/04/2024` -> `2024-04-01`; `12.07.2024` -> `2024-07-12`). Apply this to the ambiguous token
  even if another date in the same document uses a different numeric order. Omission, a different
  unresolved interpretation, or an ambiguity warning is a blocking defect;
...
- Treat a fully reconciled contiguous pallet-number partition as the narrow exception to the
  shipment-total rule above. When ranges begin at 1, remain gapless/non-overlapping, partition the
  pallet total printed for exactly one identified container, and each range heads its own inner
  package/product/measure row, require one cargo group per range. Each group carries the inclusive
  outer `PALLET` quantity plus its printed inner package quantity/type and row measures; its
  `single_package_level` allocation points only to the outer pallet fact. The shipment pallet total
  must not appear as a duplicate package fact. Do not demand a synthesized containment sentence in
  `additionalInformation`: the range evidence, shared group, and package-level allocation preserve
  the supported target semantics. This exception does not apply to overlapping, gapped, reset, or
  multi-container ranges without explicit linkage;
...
- Reject party names, addresses, postal lines, and contacts placed in
  ...
  semantic type.
- Treat `NO REF`, `N/A`, `NONE`, `NIL`, and equivalent empty placeholders as absence, not as
  `forwardingAndExportReferences` values.
- Audit every dangerous-goods attribute that the frozen target can represent, including UN number,
  hazard category, subsidiary hazard category, an explicitly headed flashpoint, and handling or
  stowage codes. The target intentionally has no standalone proper-shipping-name or packing-group
  field. Those facts may be noted as `schema_cannot_represent`, but warning completeness is not a
  training-target defect: never force them into `description`, `additionalInformation`, or a
  non-existent correction path. A printed class `2` or `2.1` canonically maps to the readable
  category `GASES`; the numeric subclass is not preserved in a separate target field.
...
- Accept `flashPoint` from an explicit `FLASH POINT`/`FLASH PT`/`FLP` label, or from a printed
  temperature with `C.C.`, `C-CC`, or `CLOSED CUP` notation in the same local dangerous-goods
  block. Do not accept an ordinary cargo, storage, or reefer temperature.
...
- Treat a completed named, non-order consignee as a straight B/L and therefore
  `negotiability=non_negotiable`. Generic preprinted positive-original, surrender,
  one-accomplished/others-void, and `delivered unto order or assigns` boilerplate does not override
  that completed consignee. Require the named consignee (and any applicable conditional term) as
  evidence rather than `NON-NEGOTIABLE COPY` alone. An explicitly completed `TO ORDER` consignee
  remains `negotiable`; explicit sea-waybill/express terms still govern their document.
...
## Audited explicit-origin clarification (v10)

Apply this rule to every review:

- In a one-cargo-group document, explicit goods/marks wording `MADE IN <country>` requires
  `cargoGroups[0].origin.name` with the country exactly as printed. This is cargo wording, not a
  party-locality inference. Reject its omission, correction to a code, or replacement from PDF-only
  text.
